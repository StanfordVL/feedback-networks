...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_bn3resSkip', 'nn.LSTM')
   ...
   if self.step == 1 then
      ...
      if self.step > 2 then
        if self.step % 2 == 1 then
          ...
          -- relu = nn.ReLU()
          -- prevOutput = relu:forward(prevOutput:float()):cuda() + relu:forward(self.prevOutput:float()):cuda()
          -- prevOutput = relu:forward(prevOutput:double()):cuda() + relu:forward(self.prevOutput:double()):cuda()
        else
          -- prevOutput = prevOutput
          -- prevOutput = relu:forward(prevOutput:float()):cuda()
          -- prevOutput = relu:forward(prevOutput:double()):cuda()
          input = input
        end
      end
   end
