...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_bn_slim4', 'nn.LSTM')
...
function ConvLSTM:buildGate()
   ...
   :add(nn.SpatialConvolution(self.outputSize, self.outputSize, self.km, self.km, 1, 1, self.padm, self.padm):noBias())
   :add(nn.SpatialBatchNormalization(self.outputSize))
...
function ConvLSTM:buildGate2()
   ...
   if self.p1 ~= 0 then
     input2gate = nn.Sequential()
       ...
       :add(nn.SpatialConvolution(self.inputSize, self.outputSize,  1, 1, self.stride, self.stride, 0, 0))
       :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
     output2gate = nn.Sequential()
       :add(nn.Dropout(self.p1,false,false,true,self.mono))
       :add(nn.SpatialConvolution(self.outputSize, self.outputSize, 1, 1,           1,           1, 0, 0):noBias())
       :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
   else
     input2gate = nn.Sequential()
       :add(nn.SpatialConvolution(self.inputSize, self.outputSize,  1, 1, self.stride, self.stride, 0, 0))
...
function ConvLSTM:buildForgetGate()
   self.forgetGate = self:buildGate2()
...
function ConvLSTM:buildcellGate()
   ...
   if self.p2 ~= 0 then
     input2gate = nn.Sequential()
       ...
       :add(nn.SpatialConvolution(self.inputSize, self.outputSize,  1, 1, self.stride, self.stride, self.padc, self.padc))
       :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
     output2gate = nn.Sequential()
       :add(nn.Dropout(self.p2,false,false,true,self.mono))
       :add(nn.SpatialConvolution(self.outputSize, self.outputSize, 1, 1, 1, 1, self.padm, self.padm):noBias())
       :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
   else
     input2gate = nn.Sequential()
       :add(nn.SpatialConvolution(self.inputSize, self.outputSize,  1, 1, self.stride, self.stride, self.padc, self.padc))
