...
function checkpoint.save(epoch, model, optimState, isBestModel, opt)
   ...
   -- clear previous epoch's saved file
   if epoch ~= 1 then
      local prev = epoch - 1
      local prev_model = 'model_' .. prev .. '.t7'
      local prev_optim = 'optimState_' .. prev .. '.t7'
      os.remove(paths.concat(opt.save, prev_model))
      os.remove(paths.concat(opt.save, prev_optim))
   end
   ...
   if isBestModel then
      ...
      torch.save(paths.concat(opt.save, 'latest_best.t7'), {
         epoch = epoch,
         modelFile = modelFile,
         optimFile = optimFile,
      })
