...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_resnet', 'nn.LSTM')
...
function ConvLSTM:updateOutput(input)
   ...
   if self.step == 1 then
      ...
      if self.step > 2 then 
        ...
        --      :add(nn.SpatialConvolution(self.outputSize, self.inputSize, 1,1,1,1))
        -- input = cTrans:forward(self.prevOutput:float()):cuda()
        input = self.prevOutput
      end
   end
   --print(#prevOutput)
   --print(#prevCell)
