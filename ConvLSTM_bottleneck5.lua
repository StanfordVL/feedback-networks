...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_bottleneck5', 'nn.LSTM')
...
function ConvLSTM:buildGate()
   ...
   if self.p1 ~= 0 then
        input2gate = nn.Sequential()
                  ...
                  :add(cudnn.SpatialConvolution(self.outputSize,  self.outputSize, self.kc, self.kc, self.stride, self.stride, self.padc, self.padc))
        ...
        input2gate = nn.Sequential()
                  :add(cudnn.SpatialConvolution(self.inputSize, self.outputSize, 1, 1, self.stride, self.stride, 0, 0))
                  :add(nn.SpatialBatchNormalization(self.outputSize))
                  :add(cudnn.ReLU(true))
                  :add(cudnn.SpatialConvolution(self.outputSize, self.outputSize, self.kc, self.kc, 1, 1, 1, 1))
                  :add(nn.SpatialBatchNormalization(self.outputSize))
                  :add(cudnn.ReLU(true))
                  :add(cudnn.SpatialConvolution(self.outputSize, self.outputSize, 1, 1, 1, 1, 0, 0))
                  :add(nn.SpatialBatchNormalization(self.outputSize))
        output2gate = nn.Sequential()
                  :add(cudnn.SpatialConvolution(self.outputSize, self.outputSize, 1, 1, 1, 1, 0, 0):noBias())
                  :add(nn.SpatialBatchNormalization(self.outputSize))
                  :add(cudnn.ReLU(true))
                  :add(cudnn.SpatialConvolution(self.outputSize, self.outputSize, self.km, self.km, 1, 1, self.padm, self.padm):noBias())
                  :add(nn.SpatialBatchNormalization(self.outputSize))
...
function ConvLSTM:buildcellGate()
   ...
   if self.p2 ~= 0 then 
      ...
      input2gate = nn.Sequential()
                   :add(cudnn.SpatialConvolution(self.inputSize, self.inputSize, 1, 1, self.stride, self.stride, 0, 0))
                   :add(nn.SpatialBatchNormalization(self.inputSize))
                   :add(cudnn.ReLU(true))
                   :add(cudnn.SpatialConvolution(self.inputSize, self.outputSize, self.kc, self.kc, 1, 1, self.padc, self.padc))
                   :add(nn.SpatialBatchNormalization(self.outputSize))
                   :add(cudnn.ReLU(true))
                   :add(cudnn.SpatialConvolution(self.outputSize, self.outputSize, 1, 1, 1, 1, 0, 0))
                   :add(nn.SpatialBatchNormalization(self.outputSize))
...
function ConvLSTM:__tostring__()
   return string.format('%s(%d -> %d, iter = %d)', torch.type(self), self.inputSize, self.outputSize, self.rho)
end
