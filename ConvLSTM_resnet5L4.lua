...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_resnet5L4', 'nn.LSTM')
...
function ConvLSTM:buildGate()
   ...
   if self.p1 ~= 0 then
        input2gate = nn.Sequential()
                  ...
                  :add(nn.SpatialBatchNormalization(self.outputSize))
                  :add(nn.ReLU(true))
                  :add(cudnn.SpatialConvolution(self.outputSize, 2*self.outputSize, self.kc, self.kc, self.stride, self.stride, self.padc, self.padc))
                  :add(nn.SpatialBatchNormalization(2*self.outputSize))
                  :add(nn.ReLU(true))
                  :add(cudnn.SpatialConvolution(2*self.outputSize, 2*self.outputSize, self.kc, self.kc,           1,           1, self.padc, self.padc))
                  :add(nn.SpatialBatchNormalization(2*self.outputSize))
        output2gate = nn.Sequential()
                  :add(nn.Dropout(self.p1,false,false,true,self.mono))
                  :add(cudnn.SpatialConvolution(2*self.outputSize, 2*self.outputSize, self.km, self.km, 1, 1, self.padm, self.padm):noBias())
                  :add(nn.SpatialBatchNormalization(2*self.outputSize))
                  :add(nn.ReLU(true))
                  :add(cudnn.SpatialConvolution(2*self.outputSize, 2*self.outputSize, self.km, self.km, 1, 1, self.padm, self.padm):noBias())
                  :add(nn.SpatialBatchNormalization(2*self.outputSize))
...
function ConvLSTM:updateOutput(input)
   ...
   if self.step == 1 then
      ...
      if self.batchSize then
         self.zeroTensor:resize(input:size(1), 2*self.outputSize,(input:size(3) + 2*self.padc -self.kc)/self.stride/2 + 1,(input:size(4) + 2*self.padc - self.kc)/self.stride/2 + 1):zero()
      else
         self.zeroTensor:resize(2*self.outputSize,input:size(2)/self.stride/2,input:size(3)/self.stride/2):zero()
      end
   else
      ...
      if self.step > 2 then 
        if self.step % 2 == 1 then
          ...
          cTrans = nn.Sequential()
                :add(nn.SpatialUpSamplingNearest(2*self.stride))
