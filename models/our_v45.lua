...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      local seqShortCut = nn.Sequential()
      ...
      local s1 = nn.ConvLSTM_resnet5_new(16, 32, T, 3, 3, 2, N/opt.nGPU)
      local s2 = nn.ConvLSTM_resnet5_new(32, 64, T, 3, 3, 2, N/opt.nGPU)

      local rnnSkip1 = nn.Sequential()
         :add(nn.ConcatTable()
            :add(s1)
            :add(seqShortCut))
         :add(nn.CAddTable(true))
      ...
      model:add(nn.Sequencer(rnnSkip1))
      model:add(nn.Sequencer(rnnSkip2))
      -- model:add(nn.Sequencer(rnnSkip2))
      -- model:add(nn.Sequencer(nn.ConvLSTM_resnet5(16, 32, T, 3, 3, 2, N/opt.nGPU)))
      -- model:add(nn.Sequencer(nn.ConvLSTM_resnet5(32, 64, T, 3, 3, 2, N/opt.nGPU)))
      ...
      model:add(nn.View(4096):setNumInputDims(3))
