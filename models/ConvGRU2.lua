...
local GRU, parent = torch.class('nn.ConvGRU2', 'nn.AbstractRecurrent')
...
function GRU:buildModel()
   ...
   if self.p ~= 0 then
      ...
      self.i2g = nn.Sequential()
                 :add(nn.SpatialConvolution(self.inputSize, 2*self.outputSize, self.kc, self.kc, self.stride, self.stride, self.padc, self.padc))
                 :add(nn.SpatialBatchNormalization(2*self.outputSize, 1e-3))

      self.o2g = nn.Sequential()
                 :add(nn.SpatialConvolution(self.outputSize, 2*self.outputSize, self.km, self.km, 1, 1, self.padm, self.padm))
                 :add(nn.SpatialBatchNormalization(2*self.outputSize, 1e-3))
   end
