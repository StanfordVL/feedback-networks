...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_bn3res(50, 100, T, 3, 3, 2, N/opt.nGPU)))
      model:add(nn.Sequencer(nn.ReLU(true)))

      model:add(nn.Sequencer(Avg(16, 16, 1, 1)))
      model:add(nn.Sequencer(nn.View(100):setNumInputDims(3)))
      model:add(nn.Sequencer(nn.Linear(100, 100)))
   else
