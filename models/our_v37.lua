...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_resnet4(16, 32, T, 3, 3, 2, N/opt.nGPU, opt.nGPU)))
      model:add(nn.Sequencer(nn.ConvLSTM_resnet4(32, 32, T, 3, 3, 1, N/opt.nGPU, opt.nGPU)))
      model:add(nn.Sequencer(nn.ConvLSTM_resnet4(32, 64, T, 3, 3, 2, N/opt.nGPU, opt.nGPU)))
      model:add(nn.Sequencer(nn.ConvLSTM_resnet4(64, 64, T, 3, 3, 1, N/opt.nGPU, opt.nGPU)))
      ...
      -- model:add(nn.Sequencer(nn.View(512):setNumInputDims(3)))
      -- model:add(nn.Sequencer(nn.Linear(512, 100)))
   else
