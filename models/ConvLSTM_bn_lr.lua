...
require 'rnn'
require 'nnlr'
local ConvLSTM, parent = torch.class('nn.ConvLSTM_bn', 'nn.LSTM')

function ConvLSTM:__init(inputSize, outputSize, rho, kc, km, stride, batchSize, lr)
   self.kc = kc
   self.km = km
...
function ConvLSTM:buildGate()
   ...
   :add(nn.SpatialConvolution(self.inputSize, self.outputSize, self.kc, self.kc, self.stride, self.stride, self.padc, self.padc)
       :learningRate('weight', self.lr)
       :learningRate('bias', self.lr)
       :weightDecay('weight', self.lr)
       :weightDecay('bias', selr.lr) 
   )
   :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3)
       :learningRate('weight', self.lr)
       :learningRate('bias', self.lr)
       :weightDecay('weight', self.lr)
       :weightDecay('bias', self.lr)
   )
