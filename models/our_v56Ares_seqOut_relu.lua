...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_bn3res(22, 44, T, 3, 3, 2, N/opt.nGPU)))
      model:add(nn.Sequencer(nn.ConvLSTM_bn3res(44, 88, T, 3, 3, 2, N/opt.nGPU)))
      model:add(nn.Sequencer(nn.ReLU(true)))

      model:add(nn.Sequencer(Avg(8, 8, 1, 1)))
      model:add(nn.Sequencer(nn.View(88):setNumInputDims(3)))
      model:add(nn.Sequencer(nn.Linear(88, 100)))
   else
