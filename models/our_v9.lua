...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_bn(32, 64,  T, 3, 3, 1, N/opt.nGPU)))
      model:add(nn.Sequencer(nn.SpatialMaxPooling(2,2,2,2)))
      model:add(nn.Sequencer(nn.ReLU(true)))
      model:add(nn.Sequencer(nn.ConvLSTM_bn(64, 128, T, 3, 3, 1, N/opt.nGPU)))
      model:add(nn.Sequencer(nn.SpatialMaxPooling(8,8)))

      model:add(nn.JoinTable(1))
      model:add(nn.View(T, -1, 128, 1, 1))
      model:add(nn.Mean(1))
      ...
      -- model:add(nn.Sequencer(ReLU()))
      -- model:add(nn.Sequencer(nn.View(4096):setNumInputDims(3)))
      -- model:add(nn.Sequencer(nn.Linear(4096, 100)))
