...
local function createModel(opt)
   ...
   local function ffblock(n, stride)
      local nInputPlane = iChannels
      iChannels = n
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(layer(ffblock, 32,  1, 2))
      model:add(layer(ffblock, 64, 1, 2))
   ...
   local function ConvInit(name)
      for k,v in pairs(model:findModules(name)) do
         ...
         local n1 = v.kW*v.kH*v.nInputPlane
         local n2 = v.kW*v.kH*v.nOutputPlane
         v.weight:normal(-math.sqrt(2/(n1+n2)),math.sqrt(2/(n1+n2)))
   ...
   local function BNInit(name)
      for k,v in pairs(model:findModules(name)) do
         -- v.weight:fill(1)
         v.weight:uniform(1, 1)
         v.bias:zero()
      end
   end
   local function LNInit(name)
      for k,v in pairs(model:findModules(name)) do
         v.weight:uniform(-0.0001,0.0001)
         v.bias:zero()
      end
   end
