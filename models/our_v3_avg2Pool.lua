...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_bn(32, 64,  T, 3, 3, 2, N/opt.nGPU, 0)))
      -- model:add(nn.Sequencer(nn.ReLU(true)))
      model:add(nn.Sequencer(nn.ConvLSTM_bn(64, 128, T, 3, 3, 2, N/opt.nGPU, 0)))
      -- model:add(nn.Sequencer(nn.ReLU(true)))
