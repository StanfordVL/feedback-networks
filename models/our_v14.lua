...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      -- model:add(nn.Sequencer(nn.ConvLSTM_bn(16, 16,  T, 3, 3, 2, N/opt.nGPU)))
      -- model:add(nn.Sequencer(nn.ReLU(true)))
      model:add(nn.Sequencer(nn.ConvLSTM_bn(16, 32,  T, 3, 3, 2, N/opt.nGPU)))
      -- model:add(nn.Sequencer(nn.ReLU(true)))
      model:add(nn.Sequencer(nn.ConvLSTM_bn(32, 64,  T, 3, 3, 2, N/opt.nGPU)))
      -- model:add(nn.Sequencer(nn.ReLU(true)))
      model:add(nn.Sequencer(nn.ConvLSTM_bn(64, 128, T, 3, 3, 2, N/opt.nGPU)))
      -- model:add(nn.Sequencer(nn.ReLU(true)))

      model:add(nn.JoinTable(1))
      -- model:add(nn.View(T, -1, 128, 2, 2))
      model:add(nn.View(T, -1, 128, 4, 4))
      model:add(nn.Mean(1))
      model:add(nn.Squeeze(1))

      -- model:add(Avg(8, 8, 1, 1))
      model:add(nn.View(2048):setNumInputDims(3))
