...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_resnet5a(16, 32, T, 3, 3, 2, N/opt.nGPU)))
      model:add(nn.Sequencer(nn.ConvLSTM_resnet5a(32, 64, T, 3, 3, 2, N/opt.nGPU)))
      model:add(nn.SelectTable(-1))

      -- model:add(nn.JoinTable(1))
      -- model:add(nn.View(T, -1, 32, 8, 8))
      -- model:add(nn.Mean(1))
