...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      local shortCut1 = nn.Sequential()
                       :add(nn.SpatialAveragePooling(1, 1, 2, 2))
                       :add(nn.Concat(2)
                           :add(nn.Identity())
                           :add(nn.MulConstant(0)))
      local shortCut3 = nn.Sequential()
                       :add(nn.SpatialAveragePooling(1, 1, 2, 2))
                       :add(nn.Concat(2)
                           :add(nn.Identity())
                           :add(nn.MulConstant(0)))
      ...
      s1 = nn.ConvLSTM_resnet52(16, 32, T, 3, 3, 2, N/opt.nGPU)
      core1 = nn.Sequential()
              :add(nn.ConcatTable()
                :add(s1)
                :add(nn.shortCutLSTM(16, 32, T, 16, 2)))
              :add(nn.CAddTable(true))

      s2 = nn.ConvLSTM_resnet52(32, 32, T, 3, 3, 1, N/opt.nGPU)
      core2 = nn.Sequential()
              :add(nn.ConcatTable()
                :add(s2)
                :add(nn.shortCutLSTM(32, 32, T, 16, 1)))
              :add(nn.CAddTable(true))

      s3 = nn.ConvLSTM_resnet52(32, 64, T, 3, 3, 2, N/opt.nGPU)
      core3 = nn.Sequential()
              :add(nn.ConcatTable()
                :add(s3)
                :add(nn.shortCutLSTM(32, 64, T, 8, 2)))
              :add(nn.CAddTable(true))

      s4 = nn.ConvLSTM_resnet52(64, 64, T, 3, 3, 1, N/opt.nGPU)
      core4 = nn.Sequential()
              :add(nn.ConcatTable()
                :add(s4)
                :add(nn.shortCutLSTM(64, 64, T, 8, 1)))
              :add(nn.CAddTable(true))

      model:add(nn.Sequencer(core1))
      model:add(nn.Sequencer(core2))
      model:add(nn.Sequencer(core3))
      model:add(nn.Sequencer(core4))
