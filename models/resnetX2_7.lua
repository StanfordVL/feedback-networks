...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(Convolution(3,50,3,3,1,1,1,1))
      model:add(SBatchNorm(50))
      model:add(ReLU(true))
      model:add(layer(basicblock, 100, 1, 2))
      model:add(layer(basicblock, 200, 1, 2))
      model:add(nn.Reshape(-1))
      -- model:add(nn.Threshold(0, 1e-6))
      -- model:add(nn.Dropout(0.5))
      model:add(nn.Linear(12800, 100))
   else
