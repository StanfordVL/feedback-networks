...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_m', 'nn.LSTM')
...
function ConvLSTM:buildGate()
   ...
   :add(nn.SpatialConvolution(self.outputSize, self.outputSize, self.kc, self.kc, self.stride, self.stride, self.padc, self.padc))
   :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
   local output2gate = nn.Sequential()
   :add(nn.SpatialConvolution(self.outputSize, self.outputSize, self.km, self.km, self.stride, self.stride, self.padm, self.padm):noBias())
   :add(nn.SpatialConvolution(self.outputSize, self.outputSize, self.km, self.km, self.stride, self.stride, self.padm, self.padm):noBias())
   :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
...
function ConvLSTM:buildcellGate()
   ...
   :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))
   local output2gate = nn.Sequential()
   :add(nn.SpatialConvolution(self.outputSize, self.outputSize, self.km, self.km, self.stride, self.stride, self.padm, self.padm))
   :add(nn.SpatialBatchNormalization(self.outputSize, 1e-3))  
