...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(Convolution(3,96,3,3,1,1,1,1))
      model:add(SBatchNorm(96))
      model:add(ReLU(true))
      model:add(layer(basicblock, 192,  1, 2))
      model:add(layer(basicblock, 384, 1, 2))
      model:add(Avg(8, 8, 1, 1))
      model:add(nn.View(384):setNumInputDims(3))
      model:add(nn.Threshold(0, 1e-6))
      model:add(nn.Dropout(0.5))
      model:add(nn.Linear(384, 100))
   else
