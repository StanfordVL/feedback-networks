...
local function createModel(opt)
   ...
   elseif opt.dataset == 'cifar100' then
      ...
      model:add(nn.Sequencer(nn.ConvLSTM_resnet4(32, 32,  T, 3, 3, 2, N/opt.nGPU, opt.nGPU)))
      model:add(nn.Sequencer(nn.ConvLSTM_resnet4(32, 64,  T, 3, 3, 2, N/opt.nGPU, opt.nGPU)))
      -- model:add(nn.SelectTable(-1))

      model:add(nn.JoinTable(1))
      model:add(nn.View(T, -1, 64, 4, 4))
      model:add(nn.Mean(1))
      ...
      model:add(nn.Sequencer(nn.View(1024):setNumInputDims(3)))
      model:add(nn.Sequencer(nn.Linear(1024, 100)))
   else
