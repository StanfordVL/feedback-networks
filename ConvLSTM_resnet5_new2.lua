...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_resnet5_new2', 'nn.LSTM')
...
function ConvLSTM:updateOutput(input)
   ...
   if self.step == 1 then
      ...
      if self.step > 2 then 
          ...
          local input1 = self.prevOutput
          -- local input2 = self.output
          -- input = cTrans:forward(input1:float()):cuda() + cTrans:forward(input2:float()):cuda()
          -- input = cTrans:forward(input1:double()):cuda() + cTrans:forward(input2:double()):cuda()
          -- input = cTrans:forward(input1:float()):cuda() + input
          input = cTrans:forward(input1:double()):cuda() + input
      end
   end
   --print(#prevOutput)
   --print(#prevCell)
