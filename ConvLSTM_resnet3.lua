...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_resnet3', 'nn.LSTM')
...
function ConvLSTM:updateOutput(input)
   ...
   if self.step == 1 then
      ...
      if self.step > 2 then 
        ...
        iTrans = nn.Sequential()
              :add(nn.SpatialAveragePooling(1,1,2,2))
        local inputOriginal = iTrans:forward(input:float()):cuda()
        local inputStar = self.prevOutput + inputOriginal
        input = cTrans:forward(inputStar:float()):cuda()
      end
   end
   --print(#prevOutput)
   --print(#prevCell)
