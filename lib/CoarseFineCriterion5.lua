...
require 'nn'
require 'rnn'
local FineCoarseCriterion, parent = torch.class('nn.FineCoarseCriterion5', 'nn.Criterion')
...
function FineCoarseCriterion:__init(criterion, fm, bm)
   ...
   self.clones_c = {}
   self.gradInput = {}
   print('[INFO] using nn.FineCoarseCriterion5')
   nStep = 8
   -- for i=1,nStep do
   --   ii  = i - 1
   --   num = ii * 2
   --   if i < 5 then
   --     print ((num/nStep), "*1.0*Fine + ", (1.0-num/nStep)*(-1.0), "*Coarse")
   --   else
   --     print ("*1.0*Fine + 0*Coarse")
   --   end
   -- end
   for i=1,nStep do
      print (1/torch.sqrt(i), '*Fine + ', (1-1/torch.sqrt(i)), '* Coarse')
   end
end
...
function FineCoarseCriterion:updateOutput(input, target_both)
   ...
   for i=1,nStep do
      ...
      -- if i < 3 then
      --   -- self.output = self.output + (num/nStep)*1.0*criterion_f:forward(input[i], target[i]) + (1.0-num/nStep)*(-1.0)*criterion_c:forward(cinput:cuda(), ctarget[i])
      --   self.output = self.output + 0*criterion_f:forward(input[i], target[i]) + (1.0)*criterion_c:forward(cinput:cuda(), ctarget[i])
      -- else
      --   self.output = self.output + 1.0*criterion_f:forward(input[i], target[i]) + 0*criterion_c:forward(cinput:cuda(), ctarget[i])
      -- end
      -- print("forward")
      ...
      for i=1,nStep do
        local criterion_f, criterion_c = self:getStepCriterion(i)
        if i < 3 then
          self.output = self.output + (0.0/torch.sqrt(i))*criterion_f:forward(input[i], target[i]) + (1.0)*criterion_c:forward(input[i]*self.fm, ctarget[i])
        else
          self.output = self.output + (1.0-1.0/torch.sqrt(i))*criterion_f:forward(input[i], target[i]) + (0.0)*criterion_c:forward(input[i]*self.fm, ctarget[i])
        end
      end
   end
   return self.output
...
function FineCoarseCriterion:updateGradInput(input, target_both)
   ...
   for i=1,nStep do
      ...
      -- if torch.min(coarseGradInput[i]) > -0.011 then
      -- if i < 3 then
        -- tableGradInput[i] = (num/nStep)*fineGradInput[i] + (1.0-num/nStep)*coarseGradInput[i]
        -- tableGradInput[i] = (num/nStep)*1.0*fineGradInput[i] + (1.0-num/nStep)*(-1.0)*coarseGradInput[i]
      --   tableGradInput[i] = 0*fineGradInput[i] + (-1.0)*coarseGradInput[i]
      -- else
      ...
      if i < 3 then
        tableGradInput[i] = (0.0/torch.sqrt(i))*fineGradInput[i] + (-1.0)*coarseGradInput[i]
      else
        tableGradInput[i] = (1.0-1.0/torch.sqrt(i))*fineGradInput[i] + (0.0)*coarseGradInput[i]
      end
   end
