...
require 'nn'
require 'rnn'
local DecayFineCriterion, parent = torch.class('nn.DecayFineCriterion3', 'nn.Criterion')
...
function DecayFineCriterion:__init(criterion, fm, bm, nStep)
   ...
   self.clones_c = {}
   self.gradInput = {}
   print('[INFO] using nn.DecayFineCriterion3')
   print ('[INFO] F-C Ratio')
   for i=1,nStep do
      print (1/(2*i-1), '*Fine + ', 0, '* Coarse')
   end
end
...
function DecayFineCriterion:updateOutput(input, target_both)
   ...
   for i=1,nStep do
      ...
      self.output = self.output + (1/(2*i-1))*criterion_f:forward(input[i], target[i]) + 0*criterion_c:forward(input[i]*self.fm, ctarget[i])
   end
...
function DecayFineCriterion:updateGradInput(input, target_both)
   ...
   for i=1,nStep do
      ...
      tableGradInput[i] = (1/(2*i-1))*fineGradInput[i] + 0*coarseGradInput[i]
   end
