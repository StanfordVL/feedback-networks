...
function DecayFineCriterion:updateOutput(input, target_both)
   ...
   for i=1,nStep do
      ...
      self.output = self.output + (1/torch.sqrt(i))*criterion_f:forward(input[i], target[i]) + 0*criterion_c:forward(input[i]*self.fm, ctarget[i])
   end
...
function DecayFineCriterion:updateGradInput(input, target_both)
   ...
   for i=1,nStep do
      ...
      tableGradInput[i] = (1/torch.sqrt(i))*fineGradInput[i] + 0*coarseGradInput[i]
   end
