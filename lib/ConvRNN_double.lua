...
function ConvRNN:buildModel()
   ...
   self.i = nn.Sequential()
       ...
       :add(nn.SplitTable(1))
       :add(nn.Sequencer(nn.ConvLSTM_bn(self.inputSize,   2*self.inputSize, T, 3, 3, 2, N/nGPU):sharedClone()))
       :add(nn.Sequencer(nn.ConvLSTM_bn(2*self.inputSize, 4*self.inputSize, T, 3, 3, 2, N/nGPU):sharedClone()))
   ...
   self.o = nn.Sequential()
       ...
       :add(nn.Replicate(T))
       :add(nn.Contiguous())
       :add(nn.View(T, -1, 4*self.inputSize, 8, 8))
       :add(nn.SplitTable(1))
       :add(nn.Sequencer(nn.ConvLSTM_bn(4*self.inputSize, 4*self.inputSize, T, 3, 3, 1, N/nGPU):sharedClone()))
       :add(nn.Sequencer(nn.ConvLSTM_bn(4*self.inputSize, 4*self.inputSize, T, 3, 3, 1, N/nGPU):sharedClone()))
       :add(nn.SelectTable(-1))
...
function ConvRNN:updateOutput(input)
   --print(#input)
   local prevOutput
   print ("rnn  step:", self.stepr)
   print ("rnn  gradstep:", self.updateGradInputStepr)
...
function ConvRNN:_updateGradInput(input, gradOutput)
   ...
   local stepr = self.updateGradInputStepr - 1
   assert(stepr >= 1)
