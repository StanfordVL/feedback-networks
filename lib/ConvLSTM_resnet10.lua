...
local ConvLSTM, parent = torch.class('nn.ConvLSTM_resnet10', 'nn.LSTM')
   ...
   if self.step == 1 then
      ...
      if self.step > 2 then 
        if self.step % 2 == 1 then
          ...
          input = cTrans:forward(inputStar:double()):cuda() + cTrans:forward(prevOutput:double()):cuda()
        else
          input = cTrans:forward(prevOutput:double()):cuda()
        end 
      end
   end
   --print(#input)
